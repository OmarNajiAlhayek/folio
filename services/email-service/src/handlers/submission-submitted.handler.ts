...
@Injectable()
export class SubmissionSubmittedHandler {
  private readonly logger = new Logger(SubmissionSubmittedHandler.name);
  ...
  async handle(event: SubmissionSubmittedEvent): Promise<HandlerOutcome> {
    if (
      event.idempotencyKey !==
      submissionSubmittedKey(event.submissionSlug, event.editor.id)
    ) {
      this.logger.warn(
    ...
    return renderAndSendCopyeditEmail(
      ...
      row,
      {
        ...
        templateVars: {
          ...
          editorQueueUrl: event.editorQueueUrl,
        },
      },
    );
  }
