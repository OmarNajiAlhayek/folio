...
@Injectable()
export class CopyeditQueriesSentHandler {
  private readonly logger = new Logger(CopyeditQueriesSentHandler.name);
  ...
  async handle(event: CopyeditQueriesSentEvent): Promise<HandlerOutcome> {
    if (
      event.idempotencyKey !==
      copyeditQueriesSentKey(event.assignmentSlug, event.round)
    ) {
      this.logger.warn(
    ...
    return renderAndSendCopyeditEmail(
      ...
      row,
      {
        ...
        template: 'copyedit-queries-sent',
