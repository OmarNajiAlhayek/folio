...
export default function PublicationDetailPage() {
  ...
  // Utility to parse and render comma-separated keywords as visual tags
  const renderKeywords = (keywordStr?: string | null, isAr = false) => {
  ...
  return (
    <main className={PAGE_SHELL_NARROW}>
      ...
      {/* Category Badges (Discipline & Article Type) */}
      ...
      <div className="mt-4 flex flex-wrap gap-x-4 gap-y-1.5 text-xs font-semibold uppercase tracking-wider text-ink/50 pb-5 border-b border-ink/[0.08] dark:border-white/[0.08]">
        {data.author?.displayName && (
          <div className="flex items-center gap-1.5">
            <svg className="size-4 text-accent-2 opacity-75" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        {data.publishedAt && (
          <div className="flex items-center gap-1.5">
            <svg className="size-4 text-accent opacity-75" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            {data.files.map((f) => (
              <li key={f.id}>
                ...
                >
                  ...
                  <svg className="size-4 shrink-0 text-ink/35 transition group-hover/file:text-accent" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
